...
donnees$orbito <- as.numeric(donnees$orbito)
...
resultats <- donnees %>%
    group_by(ses) %>%
  summarise(Moyenne_delta = mean(orbito, na.rm = TRUE))
...
ggplot(donnees) +
  aes(x = ses, y = orbito, fill = ses) +  # Ajouter 'fill = ses' pour des couleurs différentes par condition
  geom_boxplot() +
...
anova_rep <- ezANOVA(data = donnees, dv = .(orbito), wid = .(sub), within = .(ses), detailed = FALSE)
anova_rep
...
mod <- lmer(orbito ~ ses + (1|sub), data = donnees)
